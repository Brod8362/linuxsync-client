...
pub fn connection_established(name: &str) {
    send_meta_notification(format!("Connected to {}.", name).as_str());
}

pub fn connection_lost(name: &str) {
    send_meta_notification(format!("Connection to {} lost.", name).as_str());
}

pub fn connection_failed(name: &str) {
    send_meta_notification(format!("Failed to connect to {}", name).as_str());
}

pub fn connection_denied(name: &str) {
    send_meta_notification(format!("{} denied the connection.", name).as_str());
}
